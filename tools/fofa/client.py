...
- 凭据自动从 tools/fofa/fofa.ini（[fofa] email/key）或环境变量 FOFA_EMAIL/FOFA_KEY 取，不硬编码、不入库。
...
def _config_credentials():
    """从 tools/fofa/fofa.ini 读 [fofa] email/key；文件不存在或缺字段则返回 (None, None)。"""
...
class FofaClient:
    def __init__(self, email=None, key=None, base=BASE, min_interval=1.1):
        ...
        # 优先级：显式入参 > 环境变量 > 配置文件 tools/fofa/fofa.ini
        ...
        if not self.email or not self.key:
            raise FofaError(
                "缺少 FOFA 凭据：请在 tools/fofa/fofa.ini 的 [fofa] 段填 email/key"
                "（参照 tools/fofa/fofa.example.ini），或设环境变量 FOFA_EMAIL / FOFA_KEY")
        self.base = base
