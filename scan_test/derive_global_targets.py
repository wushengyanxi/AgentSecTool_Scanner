...
产出两个文件（均在 scan_test/ 下，gitignore）：
...
def latest_completed_snapshot(conn, override=None):
    ...
    if not dates:
        raise SystemExit("clawsec 库里没有任何快照，先跑 python3 -m clawsec pull")
    if override:
...
def main():
    ...
    ap.add_argument("--db", default=CLAWSEC_DB)
    args = ap.parse_args()
    ...
    # 带指纹库版本反推、结果落 scan_test/results）。先确保输出目录存在，免得 ocprobe 落盘失败。
    os.makedirs(os.path.join(OUT_DIR, "results"), exist_ok=True)
    out_jsonl = f"scan_test/results/global-{sd}.jsonl"
    command = (
        f"prober/bin/ocprobe -port {PORT} -concurrency 4000 -timeout 5s -rate 50 "
        f"--progress --skip-unreachable -fingerprints fingerprints/fingerprints.json "
