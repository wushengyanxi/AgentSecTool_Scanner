...
地名口径：只取 GeoLite2 的「标准英文」名（其中文名不可信，见 cn_names.py），
再经本地映射表 cn_names 转成简体中文；映射表未收录的英文名原样保留。

配置见 geoip.ini（[geoip] city_db 指向 .mmdb 路径）；凭据与库均不入库。
"""
...
from ..paths import GEOIP_CITY_DB, GEOIP_CONFIG, ROOT

_CONFIG = str(GEOIP_CONFIG)
_DEFAULT_DB = str(GEOIP_CITY_DB)
