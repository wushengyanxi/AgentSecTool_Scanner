...
.PHONY: prober zgrab test discover probe probe-zgrab load stats demo \
        fofa-info fofa-pull fofa-provinces fofa-export fofa-pv clean
...
load:
	python3 -m sink --db $(DB) --in results.jsonl

stats:
	python3 -m sink --db $(DB) --stats
...
fofa-pull:
	python3 -m fofa pull --db $(DB) $(if $(DELTA),--delta,--full) $(if $(MAX),--max-records $(MAX),) $(if $(BEFORE),--before $(BEFORE),) $(if $(AFTER),--after $(AFTER),) $(if $(QUERY),--query $(QUERY),)
fofa-provinces:
	python3 -m fofa provinces --db $(DB)
fofa-export:
	python3 -m fofa export --db $(DB) --out candidates.csv $(if $(LIMIT),--limit $(LIMIT),)
fofa-pv:
	python3 -m fofa province-versions --db $(DB)

clean:
	rm -f candidates.csv results.jsonl $(DB)
