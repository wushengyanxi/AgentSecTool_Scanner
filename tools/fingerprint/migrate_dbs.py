#!/usr/bin/env python3
"""一次性数据库拆库迁移：把旧 scan.sqlite 拆成工具侧采集库。

- fofa_candidates + fofa_state → tools/fofa/data/fofa.sqlite（不耗 FOFA 额度，直接 SQL 搬）
- clawsec_instances → tools/clawsec/data/clawsec.sqlite 的 clawsec_snapshots，补 snapshot_date
...
用法：
  python3 tools/fingerprint/migrate_dbs.py --old tools/fingerprint/data/scan.sqlite --clawsec-date 2026-06-14
"""
import argparse
import os
import sqlite3
...
from agentsectool_scanner.paths import CLAWSEC_DB as DEFAULT_CLAWSEC_DB
from agentsectool_scanner.paths import FOFA_DB as DEFAULT_FOFA_DB

from tools.fofa import pull as fofa_pull
from tools.clawsec import pull as clawsec_pull

FOFA_DB = str(DEFAULT_FOFA_DB)
CLAWSEC_DB = str(DEFAULT_CLAWSEC_DB)
LEGACY_SCAN_DB = os.path.join(ROOT, "tools", "fingerprint", "data", "scan.sqlite")
