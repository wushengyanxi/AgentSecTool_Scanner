...
表结构由 `progress/blocks.py` 在访问数据库时创建，默认数据库为 `data/scanner/scan_results.sqlite`。
...
```bash
python3 -m progress claim --campaign cn-2026w24 --worker worker-a
```
...
1. 调用 `python3 -m progress claim` 领取块。
2. 用 `assetprobe --type <asset_type>` 或其他扫描入口处理该块。
3. 扫描成功后调用 `progress.blocks.mark_done` 或后续补充的 CLI 标记完成。

## 验证

相关单测位于 `progress/tests/test_blocks.py`，覆盖 CIDR 切块、幂等登记、领取、完成和过期重置。
