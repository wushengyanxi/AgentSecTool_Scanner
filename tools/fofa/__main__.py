"""FOFA 工作流 CLI。
  ...
  python3 -m tools.fofa pull --delta
  python3 -m tools.fofa provinces
"""
...
from agentsectool_scanner.paths import FOFA_CANDIDATES, FOFA_DB, SCANNER_DB
...
def main(argv=None) -> None:
    ...
    p.add_argument("--db", default=str(FOFA_DB))
    ...
    pr.add_argument("--db", default=str(FOFA_DB))

    e = sub.add_parser("export", help="导出候选为 candidates.csv（喂探测器）")
    e.add_argument("--db", default=str(FOFA_DB))
    e.add_argument("--out", default=str(FOFA_CANDIDATES))
    e.add_argument("--limit", type=int, default=None)

    pv = sub.add_parser("province-versions", help="按省 × 版本统计已探测资产")
    pv.add_argument("--db", default=str(FOFA_DB))
    pv.add_argument("--scanner-db", default=str(SCANNER_DB),
