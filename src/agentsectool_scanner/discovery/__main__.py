"""发现层 CLI：主动扫描 + 被动种子 → candidates.csv。
...
例：
  ...
  sudo env PYTHONPATH=src:. python3 -m agentsectool_scanner.discovery --cidr 0.0.0.0/0 --backend masscan --rate 20000 --excludefile config/blocklist.txt

  # 叠加 FOFA 被动种子（凭据走环境变量）
  FOFA_EMAIL=... FOFA_KEY=... python3 -m agentsectool_scanner.discovery --cidr 0.0.0.0/0 --backend masscan --fofa
"""
...
from ..paths import DISCOVERY_CANDIDATES


def main(argv=None) -> None:
    ap = argparse.ArgumentParser(
        "discovery",
        description="发现层：主动扫描 + 被动种子 → candidates.csv",
    )
    ...
    ap.add_argument("--out", default=str(DISCOVERY_CANDIDATES))
