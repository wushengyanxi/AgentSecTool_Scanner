...
仅用标准库，可直接 `python3 tools/fingerprint/collect_corpus.py ...` 运行。

用法：
  # 全量（默认遍历 repo 里所有正式 vYYYY.M.D tag）
  python3 tools/fingerprint/collect_corpus.py --repo /path/to/openclaw

  # 只采指定几个版本
  python3 tools/fingerprint/collect_corpus.py --repo /path/to/openclaw --only 2026.1.8,2026.5.12
...
from agentsectool_scanner.paths import OPENCLAW_FINGERPRINTS

HERE = os.path.dirname(os.path.abspath(__file__))
DEFAULT_DB = str(OPENCLAW_FINGERPRINTS)
DEFAULT_REGISTRY = "ghcr.io/openclaw/openclaw"
REPORT_PATH = os.path.join(HERE, "output", "collect_corpus_report.json")
