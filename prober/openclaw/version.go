...
// determineVersion 取版本：优先直读 serverVersion（疏防实例），否则用隐式指纹反推。
// hits 是调用方预先用 FingerprintDB.MatchImplicit 算好的资产指纹命中版本集（与 C3 判定共用同一次匹配）。
...
//   - 未命中：全空
func determineVersion(ev Evidence, hits []string) (version, source string, candidates []string) {
	if ev.ServerVersion != "" {
